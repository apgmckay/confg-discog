...
resource "local_file" "confg_discog_template_toml" {
  ...
  filename = format("%s/%s", var.confg_discog_template_output_path, var.confg_discog_config_file)
}

resource "local_file" "confg_discog_template_bash" {
  count    = var.template_render == true ? 1 : 0
  content  = local.cgd_bash_runtime_file
  filename = format("%s/%s", var.confg_discog_template_output_path, var.confg_discog_app_config_file)
}
